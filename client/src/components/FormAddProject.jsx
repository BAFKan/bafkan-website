...
const FormAddProject = ({ isOpen, setIsOpen }) => {
  const variants = {
    hidden: { scale: 0, opacity: 0 },
    visible: { scale: 1, opacity: 1, transition: { type: "tween" } },
  };
    ...
    <>
      {isOpen && (
        <m.div
          ...
          className="absolute flex justify-center items-center w-screen h-screen bg-gray-700/20 backdrop-blur-[2px] z-20"
        >
          <m.div
            initial="hidden"
            animate="visible"
            variants={variants}
            class="bg-white p-10 relative flex flex-col rounded-2xl bg-transparent bg-clip-border text-gray-700 shadow-xl"
          >
            {/* Close Button */}
            <button
              onClick={() => setIsOpen(false)}
              className="absolute top-0 left-0 flex justify-center items-center rounded-full h-12 w-12 -m-4"
            >
